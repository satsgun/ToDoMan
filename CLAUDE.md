...
- **`todo/models.py`** — `Task` dataclass (`id`, `title`, `done`, `created_at`). `Task.from_dict()` is the only deserializer; extra keys are silently ignored.
...
All test classes patch `storage.STORAGE_PATH` with a `tempfile.TemporaryDirectory` path in `setUp`/`tearDown`. CLI command functions are called directly with `argparse.Namespace(...)` — no subprocess or parser invocation needed.
